    <section id="beranda" class="home-section paddingbot-60">
      ...
      <div class="container">
       <div class="col-lg-12">
			<form class="form-horizontal" id="form_{$acak}" url="{$host}NegosiasiUlangPenawaran" method="post" enctype="multipart/form-data">
...
<script>
		...
		$('#deal_{$acak}').bind('click',function(){
			$.get(host+'webpage/checkCaptcha/'+$('#edVRCaptcha').val(), function(resp){
				if(resp=='sama'){
					loadingna();
					$.post("{$host}NegosiasiUlangPenawaranDeal",$('#form_{$acak}').serialize(),function(r){
						if(r==1){
							winLoadingClose();
